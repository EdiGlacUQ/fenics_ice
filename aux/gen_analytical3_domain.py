...
#Sampling Mesh; Identical dimensions
domain = test_domains.analytical2(Lx, Ly, nx=501,ny=ny+1, li = 1000)
xcoord = domain.x
ycoord = domain.y
...
File(''.join([dd,'analytical3_mesh.xml'])) << mesh

v.vector()[:] = bed.flatten()
File(''.join([dd,'analytical3_mesh_bed.xml'])) <<  v

v.vector()[:] = height.flatten()
File(''.join([dd,'analytical3_mesh_height.xml'])) <<  v

v.vector()[:] = bmelt.flatten()
File(''.join([dd,'analytical3_mesh_bmelt.xml'])) <<  v

v.vector()[:] = mask.flatten()
File(''.join([dd,'analytical3_mesh_mask.xml'])) <<  v

v.vector()[:] = B2.flatten()
File(''.join([dd,'analytical3_mesh_B2.xml'])) <<  v
